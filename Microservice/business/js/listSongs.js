let Song = function(props) {
    return (
        <tr>
                ...
                <td><a key={props.id} href={'/mySongEdit?sID='+props.id} className="btn btn-info"><span className="glyphicon glyphicon-edit"></span></a></td>
                <td><a key={props.id} href={'/mySongDel?id='+props.id} className="btn btn-danger"><span className="glyphicon glyphicon-remove"></span></a></td>
        </tr>
    )
...
let ShowSongs = React.createClass({
    ...
    componentDidMount: function() {
        axios.get("/getSongs").then(results =>{
    ...
    <h2>My Songs</h2>
    <a href="/createSong" className="btn btn-success" style={{float: "right"}}>Upload New Song</a>
